...
## Описание (актуально для монитора версии 6.4.1.85)
...
* `featuresIds` массив идентификаторов функционалов и/или пар [<идентификатор функционала>, <версия>],
которые должны быть проверены. Функционал проверяется на права доступа для пользователя, а, так же, если
указана версия - на лицензию. Метод не использует лицензию, а только проверяет её валидность. (см. *Лицензирование*)
* `session` идентификатор сессии пользователя (см. *`pickKServerInfo`*) или объект `request` (IncomingMessage)
* Returns: \<Promise\>.
